...
public sealed class WorkflowTransitionService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var previousStatus = string.IsNullOrWhiteSpace(entity.Status) ? "Open" : entity.Status;
        _workflowIntegrityService.EnsureTransitionAllowed("Case", previousStatus, nextStatus);

        entity.Status = nextStatus;
        entity.UpdatedAtUtc = DateTime.UtcNow;

        await UpsertScenarioSnapshotCaseStatusAsync(caseId, nextStatus, cancellationToken);

        _dbContext.WorkflowAuditEntries.Add(new WorkflowAuditEntryEntity
        {
            Id = Guid.NewGuid(),
            EntityType = "Case",
            EntityId = caseId,
